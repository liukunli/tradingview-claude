"""
回测引擎模块 (Backtest Engine Strategy)。

知识点 (Knowledge Points):
1.  **外观模式 (Facade Pattern)**:
    *   回测引擎作为一个“外观”类，将数据加载 (DataLoader)、组合管理 (PortfolioManager)、绩效评估 (PerformanceEvaluator) 等复杂的子系统封装起来。
    *   用户只需调用简单的 `run()` 接口即可完成复杂的回测任务，无需了解内部细节。

2.  **事件驱动 vs 向量化回测 (Event-Driven vs Vectorized)**:
    *   本引擎采用**逐日循环 (Daily Loop)** 的方式，接近事件驱动回测。
    *   相比向量化回测（一次性计算所有信号和收益），这种方式更灵活，能处理复杂的调仓逻辑、动态止损、滑点和冲击成本。

3.  **交易成本模型 (Transaction Cost Model)**:
    *   **佣金 (Commission)**: 双向收取 (买卖都收)，通常为万分之三。
    *   **印花税 (Stamp Duty)**: 仅卖出收取，通常为千分之一。
    *   **滑点 (Slippage)**: 模拟成交价劣势（买入更贵，卖出更便宜），用于反映市场深度不足或冲击成本。

4.  **RankIC (秩相关系数)**:
    *   衡量因子有效性的核心指标。计算因子排名与未来收益排名之间的 Spearman 相关系数。
    *   IC > 0.05 通常被认为是有效因子；IC > 0.1 是强因子。

5.  **回测流程 (Backtesting Workflow)**:
    *   **盘前 (Pre-Market)**: 计算因子值，生成选股信号。
    *   **盘中 (Intra-Day)**: 判断是否调仓。如果是，执行买卖操作，扣除成本，更新持仓。
    *   **盘后 (Post-Market)**: 结算当日盈亏，更新账户资金，计算当日 IC。
"""
...
class BacktestEngine:
    ...
    def _calculate_portfolio_return(
        ...
        if enable_cost and is_rebalance_day and rebalance_info is not None:
            ...
            # Cost Rate = Total Cost / Current Capital
