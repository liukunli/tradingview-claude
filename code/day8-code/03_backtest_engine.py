...
# Factor Calculator: 负责计算技术因子(如动量)
from factor_calculator import FactorCalculator
# Signal Generator: 负责生成交易信号(选股)
...
class BacktestEngine:
    ...
    功能: 将数据、因子、信号、交易、评估等模块串联起来,执行完整的历史回测。
    ...
    5) 因子预热: 回测首日的因子计算需要向前回溯 period 天的历史数据。
    ...
    def __init(
        ...
        # 2. 初始化因子计算器 (依赖 loader 获取数据)
        self.calculator = FactorCalculator(self.loader)
        
        # 3. 初始化信号生成器 (依赖 calculator 计算因子)
        self.signal_gen = SignalGenerator(self.loader, self.calculator)
        
        # 4. 初始化投资组合管理器 (管理资金和持仓)
    ...
    def run(
        ...
        Args:
            start_date: 回测起始日期
            end_date: 回测结束日期
            factor_name: 因子名称 (目前仅支持单一因子排序)
            factor_period: 因子计算周期 (如 20日动量)
        ...
        print(f"📊 因子: {factor_name} (周期 {factor_period})")
        print(f"📊 选股数量: {top_n}")
        ...
        # 2. 预先准备因子数据(如果不存在则现场计算)
        # 避免每日临时计算带来的重复IO与性能问题
        self._prepare_factors(backtest_dates, factor_period)
    ...
    def _run_daily_backtest(
        ...
        for i, date in enumerate(backtest_dates):
            # 1) 盘前: 生成选股信号 (使用 T-1 收盘后的数据)
            selected_stocks = self._generate_signal(date, top_n)
    ...
    def _generate_signal(self, date: str, top_n: int) -> list:
        # 选股信号由 SignalGenerator 负责
        return self.signal_gen.generate_daily_signal(
            date=date,
            top_n=top_n,
            filter_limit=True
        )
    ...
    def _record_daily_ic(
        ...
        factor_series = factor_df.set_index('code')['momentum_20']
    ...
    def _prepare_factors(self, backtest_dates: List[str], period: int):
        """
        准备因子数据 (详细处理流程)
        
        核心逻辑:
        回测依赖每日的因子数据。为了提高效率,我们将计算好的因子保存为 CSV。
        此函数检查因子是否已存在,如果缺失,则自动触发批量计算。
        
        注意: 因子计算需要"预热" (Cold Start)。
        例如: 计算 3月1日 的 20日动量, 需要用到 2月 的历史数据。
        因此, 计算区间必须比回测区间"提前" period 天。
        """
        print("\n📊 检查因子数据...")

        # 1. 检查是否存在: 取回测的第一天, 尝试读取因子文件
        first_date = backtest_dates[0]
        
        # load_factor 内部会检查文件是否存在, 如果不存在或为空, 返回 Empty DataFrame
        if self.calculator.load_factor(first_date).empty:
            print("⚠️  因子数据不存在(或缺失第一天), 触发全量计算...")
            ...
            # 4. 确定"计算起始点" (向前回溯 period 天)
            # 为什么? 因为 Momentum_20 需要前20天的收盘价计算 ROI。
            # 如果不回溯, 回测第一天的因子值将是 NaN。
            # max(0, ...) 是防止索引越界 (比如回测从第1天就开始)
            calc_start_idx = max(0, start_idx - period)
            
            # 5. 确定"计算结束点" (回测最后一天)
            calc_end_idx = trade_dates.index(backtest_dates[-1])
            
            # 6. 生成需要计算的日期列表
            # 切片包含 start, 不包含 end, 所以 end_idx 需要 +1
            calc_dates = trade_dates[calc_start_idx : calc_end_idx + 1]
            
            print(f"   需要计算的日期范围: {calc_dates[0]} 至 {calc_dates[-1]} (共 {len(calc_dates)} 天)")

            # 7. 逐日计算循环
            for i, date in enumerate(calc_dates):
                # 调用 Calculator 计算当日因子 (DataFrame: code, momentum_20)
                factor_df = self.calculator.calculate_momentum_daily(date, period)
                
                # 8. 保存结果
                if not factor_df.empty:
                    # 构造保存路径: data/factors/2020-03-01.csv
                    save_path = self.calculator.factor_dir / f'{date}.csv'
                    # 保存为 CSV, 不保留索引 (index=False)
                    factor_df.to_csv(save_path, index=False)

                # 9. 打印进度条 (每50天打印一次, 避免刷屏)
                if (i + 1) % 50 == 0:
                    print(f"  ⏳ 进度: {i + 1}/{len(calc_dates)}")

            print("✅ 因子计算完成!")
        else:
            print("✅ 因子数据已存在, 跳过计算 (直接读取缓存)")
...
if __name__ == '__main__':
    ...
    report = engine.run(
        ...
        end_date='2020-12-31',
        factor_name='momentum_20', # 测试 20日动量因子
        factor_period=20,          # 因子计算窗口
        top_n=10,                  # 每日持有前 10 只
        rebalance_freq=5,          # 每 5 天调仓一次
        enable_cost=True,          # 启用成本计算
        calculate_ic=True          # 启用 IC 计算
    )
