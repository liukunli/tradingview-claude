...
def _stable_quote_url(base, symbols_str, params):
    """stable/quote (single) or stable/batch-quote (multi, post-Aug 2025)"""
    if "," in symbols_str:
        # batch: stable/batch-quote?symbols=A,B,C
...
def _stable_eod_url(base, symbols_str, params):
    """stable/historical-price-eod/full?symbol=TSLA&limit=260  (new API, post-Aug 2025)"""
    params["symbol"] = symbols_str
    # new endpoint uses 'limit' not 'timeseries'
    params["limit"] = params.pop("timeseries", 260)
    return base, params


def _stable_hist_url(base, symbols_str, params):
    """stable/historical-price-full?symbol=^GSPC&timeseries=80  (legacy stable)"""
    params["symbol"] = symbols_str
    return base, params


def _v3_hist_url(base, symbols_str, params):
    """api/v3/historical-price-full/^GSPC?timeseries=80  (legacy v3)"""
    return f"{base}/{symbols_str}", params
...
_FMP_ENDPOINTS = {
    ...
    "historical": [
        # New stable endpoint (accounts created after Aug 2025)
        ("https://financialmodelingprep.com/stable/historical-price-eod/full", _stable_eod_url),
        # Legacy endpoints (accounts created before Aug 2025)
    ...
    def _request_with_fallback(self, endpoint_key, symbols_str, extra_params=None):
        ...
        for i, (base_url, url_builder) in enumerate(endpoints):
            ...
            if endpoint_key == "historical":
                # New stable endpoint returns a flat list [{symbol, date, open, ...}, ...]
                # Normalize to v3-compatible shape {"symbol": ..., "historical": [...]}
                if isinstance(data, list) and len(data) > 0 and "date" in data[0]:
                    sym = data[0].get("symbol", symbols_str)
                    normalized = {"symbol": sym, "historical": data}
