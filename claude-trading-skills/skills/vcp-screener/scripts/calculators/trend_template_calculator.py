...
def calculate_trend_template(
    ...
    # When RS rank data is unavailable (SPY history failed), exclude this
    # criterion from scoring rather than auto-failing it.
    c7_pass = False
    if rs_rank is not None:
        ...
        criteria["c7_rs_rank_above_70"] = {
            "passed": None,  # None = excluded, not failed
            "detail": "RS Rank unavailable (SPY data missing) — criterion excluded from score",
        }

    # Raw score: score active (non-None) criteria only
    active = {k: v for k, v in criteria.items() if v["passed"] is not None}
    passed_count = sum(1 for v in active.values() if v["passed"])
    n_active = len(active)
    raw_score = round((passed_count / n_active * 100) if n_active else 0, 1)
    raw_score = min(100, raw_score)

    # Pass threshold: 85+ (equivalent to 6/7 criteria)
    passed = raw_score >= 85
