...
def parse_arguments():
    ...
    parser.add_argument(
        "--api-key", help="Alpaca API key (defaults to ALPACA_API_KEY environment variable)"
    )
    parser.add_argument(
        "--api-secret", help="Alpaca API secret (defaults to ALPACA_API_SECRET environment variable)"
    )
    parser.add_argument(
        "--max-candidates",
        type=int,
...
def main():
    ...
    try:
        client = AlpacaClient(api_key=args.api_key, api_secret=args.api_secret)
